...
class TestDeliveryCorreios(TransactionCase):
    def setUp(self):
        ...
        product_uom = {
            "name": "UOM",
            "category_id": self.env["product.uom.categ"].create({"name": "Unity"}).id,
            "uom_type": "reference",
            "active": True,
            "rounding": 0.00100,
        }
        ...
        self.sale_order_line = self.env["sale.order.line"].create(sale_order_line)
        self.sale_order.write(
            {"order_line": [(4, self.sale_order_line.id, 0)],}
        )

    @patch(
        "odoo.addons.delivery_correios.models.delivery.\
...
check_for_correio_error"
    ...
    def test_action_get_correio_services(self, services, erro):
        ...
        with open(busca_cliente, "r") as correio_return_xml:
            services.return_value = objectify.fromstring(correio_return_xml.read())
        erro.return_value = None
        self.delivery.action_get_correio_services()
        servicos = self.env["delivery.correios.service"].search(
            [("code", "=", "40096")]
        )
        self.assertTrue(len(servicos) == 1, "Número de serviços: %d " % len(servicos))
...
calcular_preco_prazo"
    ...
    def test_correios_get_tracking_link(self, eventos, preco, erro):
        ...
        with open(get_eventos, "r") as correios_eventos:
            eventos.return_value = objectify.fromstring(correios_eventos.read())
        with open(calcular_preco_prazo, "r") as correio_return_xml:
            preco.return_value = objectify.fromstring(correio_return_xml.read())
        erro.return_value = None
        move_line = [
        ...
        self.assertEqual(evento.local_origem, u"CTCE MACEIO - 57060971, MACEIO/AL")
