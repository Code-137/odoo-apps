...
STATE = {"em_digitacao": [("readonly", False)]}
...
class NfeReboque(models.Model):
    ...
    fiscal_document_id = fields.Many2one(
        "l10n_br_fiscal.document", string="NFe"
    )
...
class FiscalDocument(models.Model):
    ...
    placa_veiculo = fields.Char(
        string="Placa Veículo", readonly=True, states=STATE
    )
    uf_veiculo = fields.Char(string="UF Veículo", readonly=True, states=STATE)
    rntc = fields.Char(string="RNTC", readonly=True, states=STATE)
    ...
    def get_ide(self, nfe, operacao):
        ...
        return dict(
            ...
            document_key=num_controle,
            document_number=numero_nfe,
            authorization_date=data_emissao.astimezone(pytz.utc).replace(
                tzinfo=None
            ),
            # data_entrada_saida=dt_entrada_saida,
            # ind_dest=str(indicador_destinatario),
            ambiente=ambiente,
            edoc_purpose=finalidade_emissao,
            state_edoc="imported",
            # fiscal_position_id=fpos.id,
            operation_name=natureza_operacao,
        )
    ...
    def get_ICMSTot(self, nfe):
        ...
        return dict(
            amount_icms_base=get(ICMSTot, "vBC"),
            amount_icms_value=get(ICMSTot, "vICMS"),
            # valor_icms_deson=get(ICMSTot, 'vICMSDeson'),
            amount_icmsst_base=get(ICMSTot, "vBCST"),
            amount_icmsst_value=get(ICMSTot, "vST"),
            amount_price_gross=get(ICMSTot, "vProd"),
            # valor_produtos=get(ICMSTot, 'vProd'),
            amount_freight_value=get(ICMSTot, "vFrete"),
            amount_insurance_value=get(ICMSTot, "vSeg"),
            amount_discount_value=get(ICMSTot, "vDesc"),
            # valor_ii=get(ICMSTot, 'vII'),
            amount_ipi_value=get(ICMSTot, "vIPI"),
            amount_pis_value=get(ICMSTot, "vPIS"),
            amount_confins_value=get(ICMSTot, "vCOFINS"),
            amount_total=get(ICMSTot, "vNF"),
            amount_estimated_tax=get(ICMSTot, "vTotTrib"),
        )

    def get_retTrib(self, nfe):
        retTrib = nfe.NFe.infNFe.total.retTrib
        return dict(
            amount_pis_wh_value=retTrib.vRetPIS,
            amount_cofins_wh_value=retTrib.vRetCOFINS,
            amount_csll_wh_value=retTrib.vRetCSLL,
            amount_irrf_wh_value=retTrib.vIRRF,
            amount_inss_wh_value=retTrib.vRetPrev,
        )
        ...
        if hasattr(nfe.NFe.infNFe, "cobr"):
            ...
            if len(cobr) and hasattr(cobr, "dup"):
                ...
                for dup in cobr.dup:
                    ...
                    duplicatas.append(
                        (0, None, remove_none_values(duplicata))
                    )
    ...
    def get_protNFe(self, nfe, company_id):
        ...
        if (
            ...
            and company_id.cnpj_cpf in protNFe.chNFe
        ):
            return dict(
                document_key=protNFe.chNFe,
                data_autorizacao=parser.parse(
                    str(nfe.protNFe.infProt.dhRecbto)
                ),
                status_description=protNFe.xMotivo,
                authorization_protocol=protNFe.nProt,
                status_code=protNFe.cStat,
            )
    ...
    def create_invoice_eletronic_item(
        ...
        if (
            not product
            and item.prod.cEAN
            and str(item.prod.cEAN) != "SEM GTIN"
        ):
        ...
        uom_id = (
            self.env["uom.uom"]
            .search([("name", "=", str(item.prod.uCom))], limit=1)
            .id
        )
    ...
    def get_items(
        self, nfe, company_id, partner_id, supplier, product_automation
    ):
        items = []
    ...
    def import_nfe(
        ...
        invoice_dict.update(
            {
                "authorization_file_id": self.env["ir.attachment"].create(
                    {
                        "name": "NFe%08d.xml" % nfe.NFe.infNFe.ide.nNF,
        ...
        self.env["l10n_br_fiscal.document"].create(invoice_dict)
    ...
    def existing_invoice(self, nfe):
        ...
        invoice_eletronic = self.env["l10n_br_fiscal.document"].search(
        ...
        city_id = self.env["res.city"].search(
            [
                (
                    "ibge_code",
                    "=",
                    get(tag_nfe, ender_tag + ".cMun", str)[2:],
                ),
                ("state_id", "=", state_id.id),
            ]
        )
    ...
    def _create_product(self, company_id, supplier, nfe_item, uom_id=False):
        ...
        seq_id = int(
            params.get_param("br_nfe_import.product_sequence", default=0)
        )
        if not seq_id:
            raise UserError(
    ...
    def _get_tax(
        self, tax_domain, aliquota, company_id, tax_automation=False
    ):
    ...
    def _create_supplierinfo(
        self, item, purchase_order_line, automation=False
    ):
    ...
    def _get_company_invoice(self, nfe, partner_automation):
        ...
        if not company:
            company = (
                self.env["res.company"]
                .sudo()
                .search([("partner_id.cnpj_cpf", "=", emit_cnpj_cpf)])
            )
            if company:
        ...
        return dict(
            ...
            fiscal_operation_type=nfe_type,
    ...
    def generate_eletronic_document(self, xml_nfe, create_partner):
        ...
        invoice_dict.update(self.get_compra(nfe))
        invoice_dict.pop("destinatary", False)
        return self.env["l10n_br_fiscal.document"].create(invoice_dict)
    ...
    def _prepare_account_invoice_vals(self):
        operation = (
            "in_invoice" if self.tipo_operacao == "in" else "out_invoice"
        )
        journal_id = (
            self.env["account.move"]
            .with_context(
                default_move_type=operation,
    ...
    def generate_account_move(self):
        ...
        if self.valor_ipi:
            product = self.env.ref(
                "l10n_br_nfe_import.product_product_tax_ipi"
            )
            items.append(
                (0, 0, self.prepare_extra_line_items(product, self.valor_ipi))
            )

        if self.valor_icmsst:
            product = self.env.ref(
                "l10n_br_nfe_import.product_product_tax_icmsst"
            )
            items.append(
                (
                    0,
                    0,
                    self.prepare_extra_line_items(product, self.valor_icmsst),
                )
            )
        ...
        if self.valor_frete:
            ...
            items.append(
                (
                    0,
                    0,
                    self.prepare_extra_line_items(product, self.valor_frete),
                )
            )
        ...
        if self.valor_despesas:
            ...
            items.append(
                (
                    0,
                    0,
                    self.prepare_extra_line_items(
                        product, self.valor_despesas
                    ),
                )
            )
        ...
        if self.valor_seguro:
            product = self.env.ref(
                "l10n_br_account.product_product_insurance"
            )
            items.append(
                (
                    0,
                    0,
                    self.prepare_extra_line_items(product, self.valor_seguro),
                )
            )
